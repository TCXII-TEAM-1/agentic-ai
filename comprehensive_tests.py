...
def run_test_case(category, test_name, ticket_obj, expected_behavior):
    ...
    if original_desc != scrubbed_desc:
        print(f"  {BColors.OKGREEN}✔ PII Redacted{BColors.ENDC}")
    ...
    try:
        ...
        doc_count = len(retrieval.documents) if retrieval.documents else 0
        ...
        if expected_behavior == "ANSWER":
             if confidence >= 0.6:
                 print(f"  {BColors.OKGREEN}✔ PASSED (Answered){BColors.ENDC}")
             else:
                 print(f"  {BColors.FAIL}✘ FAILED (Too low confidence){BColors.ENDC}")

        elif expected_behavior == "REFUSE":
             if confidence >= 0.9 or "Refusal" in reasoning or "Off-topic" in reasoning:
                  print(f"  {BColors.OKGREEN}✔ PASSED (Refused){BColors.ENDC}")
             elif confidence < 0.5:
                  # Low confidence escalation is arguably acceptable for off-topic, 
                  # but we Prefer definite refusal (High Confidence Off-topic).
                  print(f"  {BColors.WARNING}⚠ WARNING (Escalated instead of Refused){BColors.ENDC}")
             else:
                  print(f"  {BColors.FAIL}✘ FAILED (Answered off-topic query){BColors.ENDC}")
        
        elif expected_behavior == "ESCALATE":
             if confidence < 0.6:
                 print(f"  {BColors.OKGREEN}✔ PASSED (Escalated){BColors.ENDC}")
             else:
                 print(f"  {BColors.FAIL}✘ FAILED (False Positive Answer){BColors.ENDC}")
